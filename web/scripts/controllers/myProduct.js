...
'use strict';
rangerApp.controller('myProductCtrl', ['$scope', '$http', '$state', 'angency', '$window', '$uibModal',
    function ($scope, $http, $state, angency, $window, $uibModal) {
        $scope.page={
            "pageSize":10, "pageNo":1, "totalCount":18
        };
        ...
        $scope.set_total = function(number){
            $scope.page.totalCount = number;
        };
        ...
        if (!$window.sessionStorage.angencyId) {
            ...
            $http({
                ...
                $scope.page.totalCount = $scope.productList.length;
                // console.log(response.data);
                // console.log($scope.page.totalCount);
            }, function (err) {
                alert("获取失败  " + err);
            });
        };
